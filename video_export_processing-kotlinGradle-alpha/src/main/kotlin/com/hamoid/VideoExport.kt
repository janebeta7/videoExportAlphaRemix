...
class VideoExport @JvmOverloads constructor(
    ...
    outputFileName: String = "p5-movie.mp4",
    img: PImage = parent.g
) {
    val VERSION = "0.2.4_janebeta7_compile_alpha_narkai"
    ...
    /**
     * Advanced. Set to true if you want to save an accompanying argb
     * *https://github.com/hamoid/video_export_processing/issues/49
     * video with the alpha channel from a PGraphics. This is useful if you
     * want to layer the video on top of other videos in a video editing
     * software while a having transparent background in your Processing output.
     *
     * @param doSaveAlpha Specify if an alpha channel video is desired
     */
    fun setSaveAlphaVideo_narkai(doSaveAlpha: Boolean) {
        Settings.saveAlphaVideo_narkai = doSaveAlpha
    }
    /**
    ...
    fun saveFrame() {
        if (img != null && img!!.width > 0) {
            ...
            if (pixelsByteRGB == null) {
                ...
                if (Settings.saveAlphaVideo_narkai) {
                    pixelsByteRGB =
                        ByteArray(img!!.pixelWidth * img!!.pixelHeight * 4)
                }
                else
                {
                    pixelsByteRGB =
            ...
            run {
                ...
                if (Settings.saveAlphaVideo_narkai) {
                    for (px in img!!.pixels) {
                        pixelsByteRGB!![byteNum++] = (px shr 24).toByte()
    ...
    protected fun startFfmpeg() {
        ...
        println("-----------ffmpeg: " + encodeSettings.toString())
